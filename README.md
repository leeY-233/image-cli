...
注意：provider 列表保存后会立即用于新的请求；其他 `.env` 配置保存后请通过管理页重启按钮或手动重启服务生效。
...
| `IMAGE_PROVIDERS` | 多 provider 配置 JSON 数组；`api_type` 默认为 `images`，需要通过 `/v1/chat/completions` 生成的 provider 设为 `chat_completions`；可选 `model` 会覆盖全局 `IMAGE_MODEL` | `[{"id":"fast","name":"Fast","api_type":"images","base_url":"https://example.com/v1","api_key":"sk-...","model":"provider-model","note":"速度优先"}]` |
...
返回中不会暴露 API Key，只会返回 provider 的 `id`、`name`、`api_type`、`base_url`、`model`、`note` 和 `api_key_configured`。
