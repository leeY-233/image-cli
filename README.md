...
1. 在左侧填写 prompt。
2. 选择比例和质量，默认质量是 `auto`。
3. 选择输出格式，默认格式是 `png`。
4. 点击生成后，右侧会显示当前 Job 进度和生成结果。
5. Count 大于 1 时，后端会尽量返回对应数量的图片；如果 provider 单次只返回 1 张，会自动补发剩余请求。
6. 生成中可以点击“后台运行”，当前任务继续在后端执行，表单会解锁，可以提交新的 prompt。
7. 顶部状态按钮会显示后台 Job 数量，点击后可以查看正在运行的任务列表，也可以取消任务。
8. 历史记录在右侧下方，点击某条历史可以打开弹窗查看图片、prompt、质量信息，并支持复用或复制 prompt。
9. 点击“选择”后可以勾选历史记录，进行全选、批量下载 ZIP 或批量删除。
...
| `3:4` | `768x1024` | 半身人像、竖版海报、商品详情图 |
| `4:3` | `1024x768` | 摄影感图片、室内场景、文章配图 |
| `9:16` | `1152x2048` | 手机壁纸、短视频封面、竖屏海报 |
| `16:9` | `2048x1152` | 宽屏壁纸、视频封面、横屏场景图 |
| `21:9` | `3840x1648` | 超宽电影感画面、游戏概念图、横幅 |
| `Custom` | 用户自定义 | 宽高需为 `16` 的倍数 |

`1:1`、`3:4` 和 `4:3` 使用 1K 长边；`9:16` 和 `16:9` 使用 2K 长边；`21:9` 使用 4K 宽边。所有预设尺寸的宽高都能被 `16` 整除。自定义尺寸限制为 `16` 到 `4096`，宽高都必须能被 `16` 整除，且总像素不能超过 `3840x2160` 的像素预算（`8,294,400` 像素）。
